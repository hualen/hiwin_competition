...
PHOTO_POSE1 = [3.00, 0.00, 0.00, 0.00, -90.00, 0.00]
PHOTO_POSE2 = [-3.00, 0.00, 0.00, 0.00, -90.00, 0.00]
...
HOME = [0.00, 0.00, 1.00, 0.00, -90.00, 0.00]
...
class ExampleStrategy(Node):
    ...
    def _state_machine(self, state: States) -> States:
        if state == States.INIT:
            ...
            pose = Twist()
            [pose.linear.x, pose.linear.y, pose.linear.z] = HOME[0:3]
            [pose.angular.x, pose.angular.y, pose.angular.z] = HOME[3:6]
            req = self.generate_robot_request(
                cmd_type=RobotCommand.Request.PTP,
                pose=pose,
                holding=True
                )
            ...
            self.get_logger().info('GO_HOME_2')
            req = self.generate_robot_request(
